...
// subtree rooted at p
public static String checkLeftSubtree(AVLTree<String, Integer>  t,Position<String,Integer> p, Comparator comp)
{
    if (t.external(p)) 
        ...
        if (comp.compare(resultLeft,"whatever") != 0 ) {
            if ( comp.compare(resultLeft,(((DictEntry<String,Integer>) (p.element())).key())) > 0 )
            	return null;
        }
        ...
        if (comp.compare(resultRight,"whatever") != 0) {
            ...
            return(resultRight);
            	...
            	return (  (String) (((DictEntry<String,Integer>) (p.element())).key())  );
        }
        else return((String) (((DictEntry<String,Integer>) (p.element())).key()));
    }
...
// subtree rooted at p
public static String checkRightSubtree(AVLTree<String, Integer>  t,Position<String,Integer> p, Comparator comp)
{
    if (t.external(p)) 
        return(new String("whatever"));
    else{
        ...
        if (comp.compare(resultRight,"whatever") != 0 ) {
            if ( comp.compare(resultRight,(((DictEntry<String,Integer>) (p.element())).key())) < 0 ) return null;
        }        
...
public static boolean checkIfBST(AVLTree<String, Integer>  treeI, Comparator comp){
...
public static void main(String[] args) throws AVLTreeException{
    ...
    case 1: // Test 1: add and find a few entries
        try {
            ...
            DictEntry e = tree.find("hello");
            int val = (Integer)e.value();
            val++;
            tree.modifyValue("hello", val);
            e = tree.find("hello");
            val = (Integer)e.value();
            val++;
            tree.modifyValue("hello", val);
            ...
            Object t = tree.find(new String("hello"));
            Object t1 = tree.find(new String("hello6"));
    ...
    case 6: //Test 6: try to change value in non-existing entry
        try {
            ...
            tree.modifyValue(new String("hello66565" ),new Integer(5) );
